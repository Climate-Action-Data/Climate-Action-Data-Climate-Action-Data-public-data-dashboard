import { Icon, StyleProps } from '@chakra-ui/react'

export type GoogleIconProps = StyleProps

export const GoogleIcon = (props: GoogleIconProps) => {
  const actualProps = {
    ...props,
    width: props?.width ?? `20px`,
    height: props?.height ?? `20px`,
  }
  return (
    <Icon viewBox="0 0 20 20" {...actualProps}>
      <path
        d="M19.8052 10.2302C19.8052 9.55044 19.7501 8.86699 19.6325 8.19824H10.2002V12.0491H15.6016C15.3775 13.291 14.6573 14.3897 13.6027 15.0878V17.5864H16.8252C18.7176 15.8448 19.8052 13.2726 19.8052 10.2302Z"
        fill="currentColor"
      />
      <path
        d="M10.1999 20.0008C12.897 20.0008 15.1714 19.1152 16.8286 17.5867L13.6061 15.088C12.7096 15.698 11.5521 16.0434 10.2036 16.0434C7.59474 16.0434 5.38272 14.2833 4.58904 11.917H1.26367V14.4928C2.96127 17.8696 6.41892 20.0008 10.1999 20.0008Z"
        fill="currentColor"
      />
      <path
        d="M10.1999 3.95805C11.6256 3.936 13.0035 4.47247 14.036 5.45722L16.8911 2.60218C15.0833 0.904587 12.6838 -0.0287217 10.1999 0.000673888C6.41892 0.000673888 2.96126 2.13185 1.26367 5.51234L4.58537 8.08813C5.37537 5.71811 7.59106 3.95805 10.1999 3.95805Z"
        fill="currentColor"
      />
      <path
        d="M4.58564 11.9173C4.16676 10.6753 4.16676 9.33044 4.58564 8.08848V5.5127H1.26395C-0.154389 8.33834 -0.154389 11.6674 1.26395 14.493L4.58564 11.9173Z"
        fill="currentColor"
      />
    </Icon>
  )
}
